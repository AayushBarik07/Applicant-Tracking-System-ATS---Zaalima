const fs = require('fs');
let content = fs.readFileSync('controllers/applicationController.js', 'utf8');
  ...
  try {
    ...
    try {
      if (req.file.originalname.toLowerCase().endsWith('.pdf')) {
        ...
        // For docx, we fallback to python microservice or just say it's not supported for AI yet
...
content = content.replace(/const createApplication = async \(req, res\) => \{[\s\S]*?res\.status\(500\)\.json\(\{ message: 'Server error' \}\);\n  \}\n\};/, replacement);
fs.writeFileSync('controllers/applicationController.js', content);
