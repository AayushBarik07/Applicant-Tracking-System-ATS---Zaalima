...
const CandidateDashboard = () => {
  ...
  const fetchMyApplications = async () => {
    ...
    const { data } = await axios.get((import.meta.env.VITE_API_URL || 'http://localhost:5000/api') + '/applications/my', {
  ...
  const respondToInterview = useMutation({
    mutationFn: async ({ id, response }) => {
      const token = localStorage.getItem('token');
      await axios.patch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/applications/${id}/interview-response`, { response }, {
        headers: { Authorization: `Bearer ${token}` }
      });
    },
    onSuccess: () => {
  ...
  const respondToOffer = useMutation({
    mutationFn: async ({ id, response }) => {
      const token = localStorage.getItem('token');
      await axios.patch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/applications/${id}/offer-response`, { response }, {
        headers: { Authorization: `Bearer ${token}` }
      });
    },
    onSuccess: () => {
