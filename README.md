...
## Architecture
The application follows a standard MERN (MongoDB, Express, React, Node) stack architecture, enhanced with a Python Microservice. The backend serves as a RESTful API, enforcing strict role-based authorization. When a file is uploaded, it is sent to Cloudinary, and the resulting URL is passed to the Python Microservice for text extraction. React Query acts as the data-fetching layer on the frontend, synchronizing server state and automatically invalidating caches.
